...
func ParseChainedContextualPos3(src []byte) (ChainedContextualPos3, int, error) {
...

	{
		...
		if L := len(src); L < n+2+arrayLengthInputCoverages*2 {
			return item, 0, fmt.Errorf("reading ChainedContextualPos3: "+"EOF: expected length: %d, got %d", n+2+arrayLengthInputCoverages*2, L)
		}
		...
		for i := range item.InputCoverages {
			offset := int(binary.BigEndian.Uint16(src[n+2+i*2:]))
			// ignore null offsets
			if offset == 0 {
				continue
			}
	...
	if L := len(src); L < n+4 {
		return item, 0, fmt.Errorf("reading ChainedContextualPos3: "+"EOF: expected length: n + 4, got %d", L)
	}
	arrayLengthLookaheadCoverages := int(binary.BigEndian.Uint16(src[n+2:]))

	{

		if L := len(src); L < n+4+arrayLengthLookaheadCoverages*2 {
			return item, 0, fmt.Errorf("reading ChainedContextualPos3: "+"EOF: expected length: %d, got %d", n+4+arrayLengthLookaheadCoverages*2, L)
		}
		...
		for i := range item.LookaheadCoverages {
			offset := int(binary.BigEndian.Uint16(src[n+4+i*2:]))
			// ignore null offsets
			if offset == 0 {
				continue
			}
	...
	if L := len(src); L < n+6 {
		return item, 0, fmt.Errorf("reading ChainedContextualPos3: "+"EOF: expected length: n + 6, got %d", L)
	}
	arrayLengthSeqLookupRecords := int(binary.BigEndian.Uint16(src[n+4:]))

	{

		if L := len(src); L < n+6+arrayLengthSeqLookupRecords*4 {
			return item, 0, fmt.Errorf("reading ChainedContextualPos3: "+"EOF: expected length: %d, got %d", n+6+arrayLengthSeqLookupRecords*4, L)
		}
		...
		for i := range item.SeqLookupRecords {
			item.SeqLookupRecords[i].mustParse(src[n+6+i*4:])
		}
...
func ParseChainedSequenceRule(src []byte) (ChainedSequenceRule, int, error) {
...

	{
		...
		if L := len(src); L < n+2+arrayLength*2 {
			return item, 0, fmt.Errorf("reading ChainedSequenceRule: "+"EOF: expected length: %d, got %d", n+2+arrayLength*2, L)
		}

		item.InputSequence = make([]uint16, arrayLength) // allocation guarded by the previous check
		for i := range item.InputSequence {
			item.InputSequence[i] = binary.BigEndian.Uint16(src[n+2+i*2:])
		}
		n += arrayLength * 2
	}
	if L := len(src); L < n+4 {
	...
	arrayLengthLookaheadSequence := int(binary.BigEndian.Uint16(src[n+2:]))

	{

		if L := len(src); L < n+4+arrayLengthLookaheadSequence*2 {
			return item, 0, fmt.Errorf("reading ChainedSequenceRule: "+"EOF: expected length: %d, got %d", n+4+arrayLengthLookaheadSequence*2, L)
		}
		...
		for i := range item.LookaheadSequence {
			item.LookaheadSequence[i] = binary.BigEndian.Uint16(src[n+4+i*2:])
		}
		n += arrayLengthLookaheadSequence * 2
	}
	if L := len(src); L < n+6 {
		return item, 0, fmt.Errorf("reading ChainedSequenceRule: "+"EOF: expected length: n + 6, got %d", L)
	}
	arrayLengthSeqLookupRecords := int(binary.BigEndian.Uint16(src[n+4:]))

	{
		...
		if L := len(src); L < n+6+arrayLengthSeqLookupRecords*4 {
			return item, 0, fmt.Errorf("reading ChainedSequenceRule: "+"EOF: expected length: %d, got %d", n+6+arrayLengthSeqLookupRecords*4, L)
		}
